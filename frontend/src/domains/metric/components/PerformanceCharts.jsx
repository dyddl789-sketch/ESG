...
export default function PerformanceCharts({ type = "line", metrics = [] }) {
  ...
  if (list.length === 0) {
    return <EmptyState title="표시할 데이터가 없습니다" description="지표 데이터가 수집되면 차트가 표시됩니다." />;
  }
  ...
  if (!lineData) {
    return <EmptyState title="월별 추이 데이터가 없습니다" description="월별 수치(months)가 수집된 지표가 없습니다." />;
  }
