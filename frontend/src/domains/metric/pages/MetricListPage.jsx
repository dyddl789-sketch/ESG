...
export default function MetricListPage() {
  ...
  const tabs = ["ALL", "DRAFT", "PENDING", "REJECTED", "APPROVED"].map((v) => ({
    value: v,
    label: { ALL: "전체", DRAFT: "검토 중", PENDING: "승인 대기", REJECTED: "반려", APPROVED: "승인 완료" }[v],
    ...
    <div className="page-stack">
      ...
      <Card>
        ...
        <div className="grid-hint">
          ...
          <span>행을 클릭하면 상세 검토 화면으로 이동합니다.</span>
        </div>
        <MetricDataGrid rows={rows} onRowClick={(row) => navigate(`/manager/metrics/${row.id}`)} />
      </Card>
    </div>
  );
}
