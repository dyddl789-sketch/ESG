  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fetchedIndicators = indRes.data || [];
        ...
        if (isEditMode) {
          const detailRes = await metricApi.detail(metricId);
          const d = detailRes.data?.data ? detailRes.data.data : detailRes.data; 
          ...
          console.log("=========================================");
          console.log("📦 [최종 마감 확인] 백엔드 응답 데이터 원본(d):", d);
          console.log("=========================================");

          // 지표 ID 매칭
          const targetIndicator = fetchedIndicators.find(
            ind => String(ind.code || ind.indicatorCode).trim() === String(d.indicatorCode).trim()
          );
          const matchedIndicatorId = targetIndicator ? String(targetIndicator.id) : "";

          // 사업장 ID 매칭
          const targetFacility = fetchedFacilities.find(
            fac => String(fac.facility_name || fac.name).trim() === String(d.facility).trim()
          );
          const matchedFacilityId = targetFacility ? String(targetFacility.id) : "";
          ...
          setFormData({
            ...
            reportingYear: Number(d.year || 2023), 
            
            /* 💡 [주기 매칭 종결] 이넘 클래스로부터 내려오는 원본 문자열을 직접 바인딩하여 100% 매칭 성공 */
            periodType: String(d.periodType || "QUARTERLY"), 
            periodValue: d.periodValue !== undefined ? Number(d.periodValue) : 1, 
            
            value: extractedValue !== "" ? String(extractedValue) : "", 
            textValue: d.textValue || "", // 💡 이제 백엔드가 보내주므로 온전하게 로드 성공!
            evidenceFileUrl: d.evidence || "" 
          });
        }
      } catch (err) {
...
const handleFileChange = async (e) => {
  ...
  const handleProcessSubmit = async (isApprovalClick = false) => {
    ...
    try {
      ...
      await Swal.fire({
        title: "처리 완료",
        text: isApprovalClick ? "데이터가 정상 처리되어 승인 요청되었습니다." : "데이터 내용이 임시 저장되었습니다.",
        icon: "success",
              ...
              <div className="form-grid">
                ...
                <div className="form-group" style={{ marginTop: "15px", borderTop: "1px dashed #eee", paddingTop: "15px" }}>
                  <label style={{ fontWeight: "bold", display: "block", marginBottom: "8px" }}>
                    증빙 파일 첨부 (PDF / 오피스 문서 / 영수증 이미지 등)
                  </label>
                  <input 
                    type="file" 
                    accept=".pdf, .png, .jpg, .jpeg, .txt, .xlsx, .xls, .docx, .hwp" 
                  ...
                  {formData.evidenceFileUrl && (() => {
                    ...
                    const cleanPath = formData.evidenceFileUrl.startsWith('/') ? formData.evidenceFileUrl : `/${formData.evidenceFileUrl}`;
                    const fileUrl = `${backendBaseUrl}${cleanPath}`;
                    const downloadApiUrl = `${backendBaseUrl}/api/files/download?fileUrl=${encodeURIComponent(cleanPath)}`;
                    ...
                    return (
                      <div style={{ marginTop: "12px", padding: "10px 14px", background: "#f4fcf7", borderRadius: "6px", border: "1px solid #e1f5e9", display: "flex", alignItems: "center", gap: "6px" }}>
                        <span style={{ fontSize: "14px", color: "#1f6b46" }}>
                          ✅ 업로드 완료: 
                          {isOfficeFile ? (
                            <a href={downloadApiUrl} style={{ textDecoration: "underline", fontWeight: "bold", marginLeft: "6px", color: "#1f6b46" }}>
                              증빙 파일 다운로드하기
                            </a>
                          ) : (
                            <a href={fileUrl} target="_blank" rel="noreferrer" style={{ textDecoration: "underline", fontWeight: "bold", marginLeft: "6px", color: "#1f6b46" }}>
                              증빙 파일 새창에서 보기
                            </a>
                          )}
                        </span>
