...
export default function ExternalBenchmarkPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setBenchmark(await externalBenchmarkApi.get());
    } catch (error) {
  ...
  const sync = async () => {
    ...
    setSyncing(true);
    try {
      setBenchmark(await externalBenchmarkApi.sync());
  ...
  const electricityChart = useMemo(() => ({
    labels: ["우리 기업 승인 누적", "2021년 동일 업종"],
    datasets: [{
  ...
  if (loading) return <div className="page-loading">외부 공공데이터 비교값을 불러오는 중입니다.</div>;
  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        title="외부 공공데이터 비교"
        description="승인 완료된 내부 전력·Scope 2·출하액 누적값을 2021년 C303 공공통계 원단위와 비교합니다."
        actions={canSync
      ...
      <div className="benchmark-source-strip">
        ...
        <div><span>내부 기준</span><b>{benchmark?.internalPeriodLabel || "승인 데이터 확인 중"}</b></div>
      </div>
      ...
      <div className="two-cols benchmark-charts">
        <Card title="출하액 기준 전력 원단위 비교" description={`${benchmark?.internalPeriodLabel || "내부 승인 누적"} · ${benchmark?.industryName || "자동차 신품 부품 제조업"}`}>
      ...
      <Card title="비교 데이터 구성" description="공공데이터는 내부 ESG 원본이 아니라 동일 업종 기준선으로 사용합니다.">
        <div className="data-role-grid">
          <article><span className="role-number">01</span><div><b>내부 승인 데이터</b><p>전력·Scope 2·출하액이 모두 승인된 최신 월까지 누적 계산합니다.</p></div></article>
